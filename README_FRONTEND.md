...
`ResultView` 의 공유 버튼은 `https://musiknot-api.duckdns.org/s/<shareId>` 를 내놓습니다. **프론트 주소가 아니라 백엔드 주소인 이유**는 GitHub Pages 가 정적이라 곡마다 다른 미리보기 메타 태그를 줄 수 없기 때문입니다. 백엔드의 `/s/` 가 메타 태그를 내려주고 사람은 다시 프론트로 넘어옵니다.

모바일이 주 사용처라 `navigator.share`(네이티브 공유 시트)를 먼저 시도하고, 없으면 `navigator.clipboard` 로 폴백합니다. 사용자가 공유 시트를 닫은 것(`AbortError`)은 실패로 취급하지 않습니다.
